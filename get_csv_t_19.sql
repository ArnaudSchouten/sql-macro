...
(
  ...
  */
  ...
  type t_settings is table of varchar2(100) index by varchar2(100);
  l_settings t_settings;
begin
  -- get the settings
  if (p_settings.exists(1))
  then
    for i in 1 .. p_settings.count()
    loop
      l_setting := trim(both '"' from p_settings(i));
      l_settings(substr(l_setting, 1, instr(l_setting, '|') - 1)) := substr(l_setting, instr(l_setting, '|') + 1);
    end loop;
  end if;

  -- set the delimiter
  if (l_settings.exists('delimiter'))
  then
    l_delimiter := nvl(l_settings('delimiter'), c_delimiter);
  end if;

  l_delimiter_record := '||''' || l_delimiter || '''||';
  -- set date / time formats
  if (l_settings.exists('date-format'))
  then
    l_date_format := nvl(l_settings('date-format'), c_date_format);
  end if;
  ...
  -- set enclosed by
  if (l_settings.exists('enclosed-by'))
  then
    l_enclosed_by := (lower(l_settings('enclosed-by')) = 'true');
  end if;

  -- set include header
  if (l_settings.exists('header'))
  then
  ...
  loop
    ...
    case
    -- strings
      when (p_tab.column(i).description.type in (dbms_tf.type_varchar2, dbms_tf.type_char, dbms_tf.type_clob)) then
        l_column := l_column_name;
      
    -- numbers
      when (p_tab.column(i).description.type in (dbms_tf.type_number, dbms_tf.type_binary_float, dbms_tf.type_binary_double)) then
        l_column := 'to_char(' || l_column_name || ')';
    ...
    -- dates
      when (p_tab.column(i).description.type = dbms_tf.type_date) then
        ...
        then
          l_column := 'to_char(' || l_column_name || ',''' || l_date_format || ''')';
        else
          l_column := 'to_char(' || l_column_name || ',''' || l_datetime_format || ''')';
        end if;
      
    -- timestamps
      when (p_tab.column(i).description.type = dbms_tf.type_timestamp) then
        l_column := 'to_char(' || l_column_name || ',''' || l_timestamp_format || ''')';
      else
        l_column := l_column_name;
    end case;
    ...
    if (l_enclosed_by)
    then
      l_column := '''' || c_enclosed_by || '''||' || l_column || '||''' || c_enclosed_by || '''';
    end if;
    ...
    then
      if (l_enclosed_by)
      then
        l_header := l_header || c_enclosed_by || l_column_name || c_enclosed_by;
      else
  ...
  l_sql := replace(replace(l_sql_template, '%HEADER%', l_header), '%RECORD%', l_record);
