--------------------------------------------------------
--  File created - Sunday-December-31-2023   
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Function SHOW_DUPLICATES
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "ARNAUD"."SHOW_DUPLICATES" (
...
begin
    ...
    return replace('select '
        ...
        then ' dup_cnt > 1 '
        when p_mode = 2
        then ' dup_cnt > 1 and rn > 1'
        when p_mode = 3
        then ' dup_cnt > 1 and rn = 1'
    end,'{l_column_list}',l_column_list);
